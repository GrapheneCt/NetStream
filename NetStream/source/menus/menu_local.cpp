...
SceVoid menu::Local::PlayerBackCb(PlayerSimple *player, ScePVoid pUserArg)
{
	delete player;
}

SceVoid menu::Local::PlayerFailCb(PlayerSimple *player, ScePVoid pUserArg)
{
...
SceVoid menu::Local::ListButtonCbFun(SceInt32 eventId, ui::Widget *self, SceInt32 a3, ScePVoid pUserData)
{
	Local *workObj = (Local *)pUserData;
	BrowserPage *workPage = workObj->pageList.back();
	LocalServerBrowser::Entry *entry = workPage->itemList->at(self->elem.hash);

	if (entry->type == LocalServerBrowser::Entry::Type_Folder)
	{
		workObj->PushBrowserPage(&entry->ref);
	}
	else if (entry->type == LocalServerBrowser::Entry::Type_SupportedFile)
	{
		new menu::PlayerSimple(workObj->browser->GetBEAVUrl(&entry->ref).c_str(), SCE_NULL, PlayerFailCb, PlayerBackCb, pUserData);
	}
}

ui::ListItem *menu::Local::ListViewCb::Create(Param *info)
{
	...
	LocalServerBrowser::Entry *entry = workPage->itemList->at(info->cellIndex);
	...
	switch (entry->type)
	{
	case LocalServerBrowser::Entry::Type_UnsupportedFile:
		tex = utils::GetTexture(tex_file_icon_unsupported);
		break;
	case LocalServerBrowser::Entry::Type_SupportedFile:
		tex = utils::GetTexture(tex_file_icon_video);
		break;
	case LocalServerBrowser::Entry::Type_Folder:
...
SceVoid menu::Local::GoToJob::ConnectionFailedDialogHandler(dialog::ButtonCode buttonCode, ScePVoid pUserArg)
{
...
SceVoid menu::Local::GoToJob::Run()
{
	SceInt32 ret = SCE_OK;
	string currentPath;
	wstring text16;
	...
	if (ret != SCE_OK)
	{
		...
		dialog::OpenError(g_appPlugin, ret, utils::GetString("msg_error_load_file"), ConnectionFailedDialogHandler, workObj);
		return;
	}
	if (workObj->firstBoot)
	{
		ui::Widget *backButton = utils::GetChild(workObj->root, button_back_page_local);
		ui::Widget *settingsButton = utils::GetChild(workObj->root, button_settings_page_local);
		thread::s_mainThreadMutex.Lock();
	...
	MenuCloseParam(false, 200.0f, Plugin::PageEffectType_SlideFromBottom))
{
	...
	browserRoot = utils::GetChild(root, plane_browser_root_page_local);
	loaderIndicator = (ui::BusyIndicator *)utils::GetChild(root, busyindicator_loader_page_local);
...
SceBool menu::Local::PushBrowserPage(string *ref)
{
	rco::Element searchParam;
	Plugin::TemplateOpenParam tmpParam;
	...
	GoToJob *job = new GoToJob("Local::GoToJob");
	job->workObj = this;
	if (ref)
	{
		job->targetRef = *ref;
...
SceBool menu::Local::PopBrowserPage()
{
